...
export default class PropertyBookingsController {
    public async index({ params, response }: HttpContext) {
        ...
        const bookings = await PropertyBooking.query()
          .where('propertyId', propertyId)
    ...
    private async checkAvailability(propertyId: number, startDate: DateTime, endDate: DateTime): Promise<boolean> {
        // Vérifier les réservations existantes
        const existingBookings = await PropertyBooking.query()
          .where('propertyId', propertyId)
          .where(builder => {
            builder.whereBetween('startDate', [startDate.toJSDate(), endDate.toJSDate()])
